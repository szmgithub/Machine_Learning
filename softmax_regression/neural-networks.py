...
def display_data(data, img_width=20):
    ...
    # ====================== YOUR CODE HERE ======================
            row_star = row*(img_height+pad)+pad
            row_end = row_star+img_height
            col_star = col*(img_width+pad)+pad
            col_end = col_star+img_width
            img_data = data[idx, :].reshape(img_height,img_width)
            disp_array[ row_star:row_end, col_star:col_end ] = img_data
            idx = idx+1
    # ===================== END OF YOUR CODE =====================
...
def nn_cost_function(nn_params, *args):
    ...
    sigmoid_z2 = 1/(1+np.exp(-z2))
    a2 = np.hstack( (b, sigmoid_z2) )
    
    z3 = np.dot( a2, Theta2.T )
    sigmoid_z3 = 1/(1+np.exp(-z3))
    Y_output = sigmoid_z3
    
    regular_item = lmb/(2*m) * ( np.sum((np.square(Theta1[:,1:]))) + np.sum((np.square(Theta2[:,1:]))) )
    J = np.sum( -1/m *( yy*np.log(Y_output) + (1-yy)*np.log(1-Y_output) ) )
#    for i in range(m):
#        for j in range(K):
#            J += (-1/m)*( yy[i,j]*np.log(Y_output[i,j]) + (1-yy[i,j])*np.log(1-Y_output[i,j]) )
    # Regularization Term
    J = J + regular_item
    # ===================== END OF YOUR CODE =====================
    return J
...
def nn_grad_function(nn_params, *args):
    ...
    z2 = np.dot( a1, Theta1.T )
    a2 = np.hstack( (b, sigmoid(z2)) )
    
    z3 = np.dot( a2, Theta2.T )
    Y_output = sigmoid(z3)
    
    # delta3 = ( Y_output - yy )*sigmoid_gradient(z3)
    delta3 = Y_output - yy
    dTheta2 = np.dot(delta3.T, a2)
    delta2 = np.dot(delta3, Theta2[:, 1:])*sigmoid_gradient(z2)
    dTheta1 = np.dot(delta2.T, a1)
    Theta1_grad = (1/m)*dTheta1
    Theta2_grad = (1/m)*dTheta2
    # Regularization Term
    regular_item1 = (lmb/m)*Theta1
    regular_item2 = (lmb/m)*Theta2
    Theta1_grad[:,1:] = Theta1_grad[:,1:] + regular_item1[:,1:]
    Theta2_grad[:,1:] = Theta2_grad[:,1:] + regular_item2[:,1:]
    # ===================== END OF YOUR CODE =====================
    # Unroll gradients
...
def rand_initialize_weigths(L_in, L_out):
    ...
    W = 2*epsilon*np.random.rand(L_out, 1 + L_in)-epsilon
    # ===================== END OF YOUR CODE =====================
    return W
